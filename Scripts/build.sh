...
if [[ ! -v SKIP_BUILD ]]; then # This is re-used for the linting job, which doesn't require a full build
  ...
  if [[ $OS == 'macos-latest' ]]; then
    ...
    pod install
    cd ..
    flutter build ipa $DEBUG --no-codesign --build-number="$BUILD_NUMBER" --build-name="$VERSION" --dart-define=cronetHttpNoPlay=false
  else
    ...
    flutter build appbundle --split-debug-info=./symbols --build-number="$BUILD_NUMBER" --build-name="$VERSION" --dart-define=cronetHttpNoPlay=false
  fi
